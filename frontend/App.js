...
export function generatePlan(raceData, profile) {
  ...
  const phases = totalWeeks <= 10
    ? [
        { name:'base',  weeks:3, kmStart:kmBase,       kmEnd:kmBase*1.2,  desnivel:Math.round((raceData.desnivel||800)*0.3)  },
        { name:'build', weeks:4, kmStart:kmBase*1.2,   kmEnd:kmBase*1.5,  desnivel:Math.round((raceData.desnivel||800)*0.6)  },
        { name:'peak',  weeks:2, kmStart:kmBase*1.5,   kmEnd:kmBase*1.6,  desnivel:Math.round((raceData.desnivel||800)*0.85) },
        { name:'taper', weeks:1, kmStart:kmBase*0.9,   kmEnd:kmBase*0.4,  desnivel:Math.round((raceData.desnivel||800)*0.2)  },
      ]
    : totalWeeks <= 12
    ? [
        { name:'base',  weeks:4, kmStart:kmBase,       kmEnd:kmBase*1.2,  desnivel:Math.round((raceData.desnivel||1000)*0.3)  },
        { name:'build', weeks:4, kmStart:kmBase*1.2,   kmEnd:kmBase*1.6,  desnivel:Math.round((raceData.desnivel||1000)*0.55) },
        { name:'peak',  weeks:3, kmStart:kmBase*1.6,   kmEnd:kmBase*1.7,  desnivel:Math.round((raceData.desnivel||1000)*0.8)  },
        { name:'taper', weeks:1, kmStart:kmBase*1.0,   kmEnd:kmBase*0.4,  desnivel:Math.round((raceData.desnivel||1000)*0.2)  },
      ]
    : totalWeeks <= 16
    ? [
    ...
    : totalWeeks <= 20
    ? [
        { name:'base',  weeks:6, kmStart:kmBase,       kmEnd:kmBase*1.3,  desnivel:Math.round((raceData.desnivel||2500)*0.25) },
        { name:'build', weeks:7, kmStart:kmBase*1.3,   kmEnd:kmBase*1.7,  desnivel:Math.round((raceData.desnivel||2500)*0.5)  },
        { name:'peak',  weeks:5, kmStart:kmBase*1.7,   kmEnd:kmBase*1.9,  desnivel:Math.round((raceData.desnivel||2500)*0.8)  },
        { name:'taper', weeks:2, kmStart:kmBase*1.1,   kmEnd:kmBase*0.4,  desnivel:Math.round((raceData.desnivel||2500)*0.2)  },
      ]
    : totalWeeks <= 24
    ? [
        { name:'base',  weeks:7, kmStart:kmBase,       kmEnd:kmBase*1.3,  desnivel:Math.round((raceData.desnivel||3000)*0.25) },
        { name:'build', weeks:8, kmStart:kmBase*1.3,   kmEnd:kmBase*1.8,  desnivel:Math.round((raceData.desnivel||3000)*0.5)  },
        { name:'peak',  weeks:6, kmStart:kmBase*1.8,   kmEnd:kmBase*2.0,  desnivel:Math.round((raceData.desnivel||3000)*0.85) },
        { name:'taper', weeks:3, kmStart:kmBase*1.2,   kmEnd:kmBase*0.4,  desnivel:Math.round((raceData.desnivel||3000)*0.2)  },
      ]
    : [
        { name:'base',  weeks:9, kmStart:kmBase,       kmEnd:kmBase*1.4,  desnivel:Math.round((raceData.desnivel||4000)*0.2)  },
        { name:'build', weeks:10,kmStart:kmBase*1.4,   kmEnd:kmBase*1.9,  desnivel:Math.round((raceData.desnivel||4000)*0.5)  },
        { name:'peak',  weeks:6, kmStart:kmBase*1.9,   kmEnd:kmBase*2.1,  desnivel:Math.round((raceData.desnivel||4000)*0.85) },
        { name:'taper', weeks:3, kmStart:kmBase*1.2,   kmEnd:kmBase*0.4,  desnivel:Math.round((raceData.desnivel||4000)*0.2)  },
      ];
  ...
  const workoutDetails = {
    ...
    longTrail:  { pct:0.40, desc:{ en:'Technical terrain, use poles',         es:'Terreno técnico, bastones' }},
    recovery:   { pct:0.12, desc:{ en:'Zone 1, very easy',                    es:'Zona 1, muy suave' }},
    backToBack: { pct:0.22, desc:{ en:'Run on tired legs from prior day',     es:'Correr con piernas cargadas' }},
  ...
  for (const phase of phases) {
    for (let i = 0; i < phase.weeks; i++) {
      ...
      weeks.push({ week: weekNum, phase: phase.name, kmTotal, desnivel: phase.desnivel, workouts });
      weekNum++;
    }
  }
...
export default function App() {
  ...
  if (screen === 'today')     return <TodayScreen {...props} onWeek={() => setScreen('week')} onRecovery={handleRecovery} />;
